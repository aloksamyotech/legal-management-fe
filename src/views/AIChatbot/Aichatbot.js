...
import { LAW_MANAGEMENT_CRM_CONTEXT } from './aiContext';

const ChatGPTClone = () => {
  const api_key = process.env.REACT_APP_APIKEy;
  const [messages, setMessages] = useState([{ id: 1, message: 'Hello! How can I assist you today?' }]);
  const [input, setInput] = useState('');
  const [previousResult, setPerviousResult] = useState('');
  const [loading, setLoading] = useState(false);

  const { isRecording, startSpeechToText, stopSpeechToText, interimResult } = useSpeechToText({ continuous: true });
  ...
  useEffect(() => {
    if (interimResult) {
      ...
      setPerviousResult(interimResult);
    }
  }, [interimResult]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const prompt = `${LAW_MANAGEMENT_CRM_CONTEXT} ${input}`;
      ...
      setMessages((prev) => [...prev.slice(0, -1), { id: 1, message: '❌ Error: Could not fetch response.' }]);
    }
    setLoading(false);
  };
