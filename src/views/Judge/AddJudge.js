...
const AddJudge = (props) => {
  const { open, handleClose, fetchJudgeData } = props;

  // -----------  validationSchema
  const validationSchema = yup.object({
    Title: yup.string().required('Title is required'),
    mobile: yup.string().matches(/^[0-9]{10}$/, "Must be 10 digits").required("Phone is required"),
  });
 ...
 const formik = useFormik({
     ...
     onSubmit: async (values) => {
       try {
         ...
         toast.error(Messages.Judge.Judge_add_Failed);
       }
     },
   });
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <Typography style={{ fontWeight: 'normal' }} variant="h3">Add New Judge</Typography>
          <Typography>
            ...
            <DialogContentText height={200} id="scroll-dialog-description" tabIndex={-1} >
              <Grid container rowSpacing={1} columnSpacing={{ xs: 0, sm: 5, md: 4 }}>
                ...
                <Grid item xs={12} sm={6}>
                  <Box mb={1}><FormLabel style={{ color: "black" }}>Contact</FormLabel></Box>
                  <TextField
                    fullWidth
                    placeholder="Mobile"
                    name="mobile"
